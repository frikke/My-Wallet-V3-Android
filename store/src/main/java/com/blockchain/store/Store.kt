...
sealed class StoreResponse<out E, out T> {
    ...
    data class Data<T>(val data: T) : StoreResponse<Nothing, T>()
    data class Error<E>(val error: E) : StoreResponse<E, Nothing>()
}
 ...
 */
sealed class KeyedStoreRequest<K> {
    data class Fresh<K>(val key: K) : KeyedStoreRequest<K>()
    data class Cached<K>(val key: K, val forceRefresh: Boolean) : KeyedStoreRequest<K>()
}