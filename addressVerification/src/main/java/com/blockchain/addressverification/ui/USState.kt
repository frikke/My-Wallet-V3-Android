...
enum class USState(
    ...
    companion object {
        fun findStateByIso(stateIso: StateIso): USState? = USState.values().find {
            it.iSOAbbreviation.equals(stateIso, ignoreCase = true)
        }

        fun findStateByANSI(stateANSI: String): USState? = USState.values().find {
            it.ANSIAbbreviation.equals(stateANSI, ignoreCase = true)
        }

        fun findStateByCode(stateCode: String): USState? =
            findStateByIso(stateCode) ?: findStateByANSI(stateCode)
    }
}
