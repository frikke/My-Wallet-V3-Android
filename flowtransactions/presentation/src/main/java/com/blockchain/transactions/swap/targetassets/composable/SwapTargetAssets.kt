...
import com.blockchain.transactions.swap.targetassets.SwapTargetAssetsIntent
import com.blockchain.transactions.swap.targetassets.SwapTargetAssetsNavigationEvent
import com.blockchain.transactions.swap.targetassets.SwapTargetAssetsViewModel
import com.blockchain.transactions.swap.targetassets.SwapTargetAssetsViewState
...
@Composable
fun SwapTargetAssets(
    sourceTicker: String,
    viewModel: SwapTargetAssetsViewModel = getViewModel(
        scope = payloadScope,
        key = sourceTicker,
    ...
    val viewState: SwapTargetAssetsViewState by viewModel.viewState.collectAsStateLifecycleAware()
    DisposableEffect(key1 = viewModel) {
        viewModel.onIntent(SwapTargetAssetsIntent.LoadData)
        onDispose { }
    }
    ...
    LaunchedEffect(navigationEvent) {
        navigationEvent?.let { navEvent ->
            when (navEvent) {
                is SwapTargetAssetsNavigationEvent.ConfirmSelection -> {
    ...
    TargetAssetsScreen(
        onSearchTermEntered = { term ->
            viewModel.onIntent(SwapTargetAssetsIntent.FilterSearch(term = term))
        },
        showModeFilter = viewState.showModeFilter,
        selectedMode = viewState.selectedModeFilter,
        onFilterSelected = {
            viewModel.onIntent(SwapTargetAssetsIntent.ModeFilterSelected(it))
        },
        assets = viewState.prices,
        accountOnClick = {
            viewModel.onIntent(SwapTargetAssetsIntent.AssetSelected(ticker = it.ticker))
        },
