...
internal fun CustodialInterestActivitySummaryItem.iconDetail(): LocalLogo {
    return when (state) {
        EarnRewardsState.COMPLETE -> when (type) {
            TransactionSummary.TransactionType.DEPOSIT -> LocalLogo.Buy
            TransactionSummary.TransactionType.INTEREST_EARNED -> LocalLogo.PassiveRewards
            TransactionSummary.TransactionType.WITHDRAW -> LocalLogo.Sell
            else -> LocalLogo.Buy
        }
        else -> LocalLogo.PassiveRewards
    }
}
...
private fun CustodialInterestActivitySummaryItem.statusStyle(): ActivityTagStyle = when (state) {
    EarnRewardsState.REFUNDED,
    EarnRewardsState.COMPLETE,
    EarnRewardsState.UNKNOWN,
